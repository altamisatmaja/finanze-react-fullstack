...
function RekapInvestasi() {
    ...
    const ReadDataSaham = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8087/inputinvestasi"
        );
        ...
        const result = bulan1.map((month) => {
          const filteredData = DataSaham.filter((data) => {
            const dataMonth = data.durasiinvestasi
              ? data.durasiinvestasi.split("-")[1]
              : null;
            return dataMonth === month || dataMonth === null;
          });

          const totalGainer = filteredData.reduce(
            (total, data) => total + parseInt(data.persentase),
            0
          );
  ...
  const totalGainAll = averageGainArray.reduce(
    (total, averageGain) => total + parseInt(averageGain),
    0
  );
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex-shrink-0">
                      <span className="text-2xl sm:text-3xl leading-none font-bold text-gray-900">
                        Statistik Portofoliomu 📈💸
                      </span>
                      <h3 className="text-base font-normal text-gray-500">
                        Tracking portofolio sahammu year-to-date
                      </h3>
                    </div>
                  ...
                  <div className="flex items-center">
                    <div className="flex-shrink-0">
                      ...
                      {DataInvestasi.length > 0 && (
                        <h3 className="text-base font-normal text-gray-500">
                          {
                            DataInvestasi[DataInvestasi.length - 1]
