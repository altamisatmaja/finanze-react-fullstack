...
function Dashboard() {
  ...
  const totalnilai = dataKeuangan.reduce(
    (tambah, data) => tambah + parseInt(data.nilai),
    0
  );
  ...
  const pemasukanData = dataKeuangan.filter(
    (data) => data.jenis === "Pemasukan"
  );

  const pengeluaranData = dataKeuangan.filter(
  ...
  const jenistotalperbulan = filterdataberdasarkanbulan.reduce(
    (total, data) => {
      const jenis = data.jenis;
      const nilai = parseInt(data.nilai);
      ...
      <div className="flex min-h-screen flex-row bg-gray-100 text-gray-800 p-5">
        ...
        <main className="main w-5/6 flex-col px-4 transition-all duration-150 ease-in md:ml-0">
          <div className="h-full items-center justify-center text-start text-3xl font-bold">
            ...
            <div className="mt-5 py-3 px-5 w-full border-collapse rounded-md bg-white text-left text-sm text-gray-500">
              <h1 className="font-Epilogue font-bold text-3xl text-black justify-center flex">
                Histori Transaksi
              </h1>
        ...
        <nav className="w-1/4 h-full sidebar">
          <div className="flex w-96 px-2 py-3">
            <p className="font-Epilogue flex font-semibold text-2xl">
              Detail Cash Flow 💸
            </p>
          </div>
          <div
            key={id}
            className="w-full mx-autopx-2 px-2 py-3 bg-white h-full rounded-lg mb-5"
          >
            <p className="font-Epilogue font-semibold text-xl">Pemasukan</p>
            {pemasukanData.slice(0, 5).map((data, id) => (
              <div className="flex justify-between mb-2">
                <p className="font-semibold items-center flex">
                  {data.kategori}
                </p>
