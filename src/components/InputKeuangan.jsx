...
function InputKeuangan() {
  ...
  const HandleSearch = async (e) => {
    e.preventDefault();
    return await axios
      .get(`http://localhost:8087/datakeuangan?q=${caridata}`)
      .then((res) => {
        setDataKeuangan(res.data);
        setCariData("");
      });
        ...
        <div>
            ...
            <div className="flex mt-2 justify-between">
              ...
              <div className="px-2">
                ...
                <input
                  ...
                  placeholder="Makanan"
                  value={kategori}
                  onChange={(e) => kategorichange(e.target.value)}
                  className="border w-56 rounded-md px-2 h-10 text-sm font-medium border-blue-200"
                />
              </div>
              <div className="px-2">
                ...
                <input
                  ...
                  placeholder="10000"
                  value={nilai}
                  onChange={(e) => nilaichange(e.target.value)}
                  className="border w-full rounded-md px-2 h-10 text-sm font-medium border-blue-200"
                />
              </div>
            </div>
            <div className="flex px-2 items-center justify-between">
              <p className="text-slate-500 text-sm font-medium pt-4">
                *Data yang Anda inputkan akan masuk kedalam database kami!
              </p>
              <div className="pt-4 flex gap-2">
                <button
                  type="submit"
                  onClick={Bersihkan}
                  className="bg-blue-900 h-8 w-24 rounded-lg px-2 mr-2 text-sm font-semibold text-white"
                >
                  Bersihkan
                </button>
                <button
                  type="submit"
                  onClick={(e) => handleSubmit(e, "Pemasukan")}
                  className="bg-blue-700 h-8 w-50 rounded-lg px-1 mr-2 text-sm font-semibold text-white"
                >
                  Tambah Pemasukan
                </button>
                <button
                  type="submit"
                  onClick={(e) => handleSubmit(e, "Pengeluaran")}
                  className="bg-blue-700 h-8 w-50 rounded-lg px-1 text-sm font-semibold text-white"
                >
                  Tambah Pengeluaran
                </button>
              </div>
          ...
          <form onSubmit={HandleSearch}>
            <div>
              ...
              <input
                value={caridata}
                onChange={(e) => setCariData(e.target.value)}
                type="text"
                name=""
                id=""
                className="font-regular text-sm"
              />
              <button className="bg-blue-900 h-8 w-24 rounded-lg px-2 mr-2 text-sm font-semibold text-white">
                Cari
              </button>
              <button
                onClick={HandleReset}
          ...
          <div>
            ...
            <button
              onClick={() => HandleFilter("Pemasukan")}
              className="bg-blue-900 h-8 w-24 rounded-lg px-2 mr-2 text-sm font-semibold text-white"
            >
              Pemasukan
            </button>
            <button
              onClick={() => HandleFilter("Pengeluaran")}
              className="bg-blue-900 h-8 w-30 rounded-lg px-2 mr-2 text-sm font-semibold text-white"
            >
              Pengeluaran
            </button>
          </div>
              ...
              datakeuangan.map((data, id) => (
                <tbody className="divide-y divide-gray-100 border-t border-gray-100">
                  <tr key={id} className="hover:bg-gray-50">
                    <th className="px-6 py-4">{data.tanggal}</th>
